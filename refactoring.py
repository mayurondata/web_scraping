...
class Scrapper:
    ...
    def combinations(self, config_file_path):
        ...
        input_parameter_for_url = [
            f"{c}{k}" for c in companies for k in keywords]
        # print(input_parameter_for_url)
        print(input_parameter_for_url)
        return input_parameter_for_url, number_of_pages

    def yahoo_scrapper(self, input_parameter_for_url, number_of_pages):
        self.input_parameter_for_url = input_parameter_for_url
        ...
        try:
            for input in input_parameter_for_url:
                ...
                yahoo_final_url = yahoo_template.format(input)
                ...
                for page in range(number_of_pages):
                    ...
                    for news in all_news:
                        ...
                        sources = news.find(
                            'span', class_='s-source mr-5 cite-co')
                ...
                dataframe = pd.concat({k: pd.Series(v)
                                      for k, v in scrapped_data.items()}, axis=1)
                print(dataframe)
                ...
                for k in range(0, len(keywords)):
                    ...
                    for page_number in range(max_pages):
                        ...
                        if response.status_code == 200:
                            ...
                            data_1 = soup.find_all(
                                "div", class_="BNeawe UPmit AP7Wnd lRVwie")
                            news_soup = soup.find_all(
                                "div", class_="BNeawe vvjwJb AP7Wnd")
                            timestamp_soup = soup.find_all(
                                "span", class_="r0bn4c rQMQod")
                            data = soup.find_all(
                                class_="Gx5Zad fP1Qef xpd EtOod pkphOe")
                            ...
                            timestt = [dateparser.parse(
                                abc[i]) for i in range(len(abc)) if i % 2 == 0]
                            ...
                            dataframe = pd.concat(
                                {k: pd.Series(v) for k, v in scrapped_Data.items()}, axis=1)
        ...
        try:
            for c in range(0, len(companies)):
                for k in range(0, len(keywords)):
                    ...
                    bing_final_url = bing_template.format(
                        input_parameter_for_url)
                ...
                while count <= number_of_pages:
                    ...
                    data = soup.find_all(
                        class_="news-card newsitem cardcommon")
                    for i in data:
...
if __name__ == '__main__':
    ...
    config_file_path = r'C:\Users\Joshi Mayuresh\Desktop\assignement_new\config.json'
    combinations = Scrapper()
    result_combinations = combinations.combinations(config_file_path)
    print(result_combinations)
# print(no_of_pages)
    yahoo_frame = Scrapper()
    result_yahoo = yahoo_frame.yahoo_scrapper()
    print(result_yahoo)
    yahoo_frame = Scrapper()
    result_yahoo = yahoo_frame.yahoo_scrapper(
        r'C:\Users\Joshi Mayuresh\Desktop\assignement_new\config.json')
    print(result_yahoo)
