...
class Scrapper:
    input_combinations_for_url =[]
    
    @staticmethod
    def inputcombinations():
        input_combinations = []
        with open(r'config.json') as fp:
            ...
            input_combinations.extend(f"{c}{k}" for c in companies for k in keywords)
            print(input_combinations)
        return input_combinations , number_of_pages
    ...
    def yahoo_scrapper(self):
        ...
        try:
            for comb in range(Scrapper.number_of_pages):
                    yahoo_template = 'https://in.news.search.yahoo.com/search?q={}'
                    yahoo_final_url = yahoo_template.format(Scrapper.input_combinations_for_url[comb])
    
                    response = requests.get(yahoo_final_url)
                    html_text = response.text
                    soup = BeautifulSoup(html_text , 'lxml')
                
                    for page in range(Scrapper.number_of_pages):
                        ...
                        for news in all_news:
                            ...
                            search_string.append(Scrapper.input_combinations_for_url[comb])
...
def main():
    ...
    Scrapper.input_combinations_for_url , Scrapper.number_of_pages = Scrapper.inputcombinations()
    result_yahoo = yahoo_frame.yahoo_scrapper()
    print(result_yahoo.columns)
    # print(result_yahoo[''])
    print(result_yahoo['Timestamp'])
    print(result_yahoo['News l ink'])
    print(result_yahoo['Search Engine'])
    print(result_yahoo['search_string'])
    print(result_yahoo['Title'])
    print(result_yahoo['Header'])
